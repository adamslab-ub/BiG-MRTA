...
from edf_taptc import tic, toc, getNextTask, getParameters

# Directory that you want to save results and outputs
output_dir = "Results_edf_taptc"
    ...
    for D in ratio_deadline_list:
        for R in robotSize_list:
            for I in instance_list:
                ...
                fileName = output_dir + '/EDF_Results_TAPTC_hungarian_'+data_name+"_Range"+str(Range)+"_Q"+str(Q)
                ...
                iRun += 1
                if iRun >= maxRun:
                    with open(output_dir + '/EDF_Results_TAPTC_hungarian_score.txt', 'w') as handle:
                        for key, val in results_score.items():
                            handle.write("%s %s\n" % (key, val))
                    with open(output_dir + '/EDF_Results_TAPTC_hungarian_time.txt', 'w') as handle:
