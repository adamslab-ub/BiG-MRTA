...
caseStudies = [10, 100, 500]; % Number of tasks
runsPerCase = 1;

regionPortion = [0.20, 0.40, 0.40];
...
for j = 1:runsPerCase
    ...
    save(strcat('FloodSim_DataSet_Hard_Dyn_n',num2str(n),'_run_',num2str(j)), 'taskData', 'depotLocation');
end
...
for i = 1:length(caseStudies)
    ...
    for j = 1:runsPerCase
        ...
        save(strcat('FloodSim_DataSet_Hard_n',num2str(n),'_run_',num2str(j)), 'taskData', 'depotLocation');
    end
end
